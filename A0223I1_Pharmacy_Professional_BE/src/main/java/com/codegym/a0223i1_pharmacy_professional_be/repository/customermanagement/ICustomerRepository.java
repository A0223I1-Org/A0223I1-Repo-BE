...
public interface ICustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Query(value="select customer.customer_id ,customer.customer_name  ,customer.age,customer.address,customer.phone_number," +
            "customer.customer_type, customer.note, customer.account_id from customer where customer.customer_name LIKE ?1%", nativeQuery = true)
    ...
    @Query(value = "select customer.customer_id, customer.customer_name,customer.age,customer.address,"+
    "customer.phone_number,customer.customer_type from customer where customer.phone_number = ?1",nativeQuery = true)
    List<CustomerDTO> getCustomersByPhone(String phoneNumber);

    @Query(value = "select customer.customer_id, customer.customer_name,customer.age,customer.address,"+
            "customer.phone_number,customer.customer_type from customer where (customer.phone_number = ?2 and customer.phone_number != ?1)",nativeQuery = true)
    List<CustomerDTO> getCustomersByPhoneUpdate(String phoneNumber, String newPhoneNumber);
